...
export async function createAnimatedStoryVideo(
  ...
  // Calcular frames baseado na duração (30fps)
  const totalFrames = duration * 30;
  
  // Frames para cada animação
  const frame2Start = 2 * 30; // 2 segundos
  const frame3Start = 4 * 30; // 4 segundos
  const ctaStart = 6.5 * 30;  // 6.5 segundos

  return new Promise((resolve, reject) => {
    ffmpeg(imagePath)
      .loop(duration)
      .videoFilter([
        // Frame 1: Zoom suave de 100% para 115% até o frame2Start (2s)
        `zoompan=z='if(lt(n,${frame2Start}),min(zoom+0.0003,1.15),1.15)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=${totalFrames}:s=1280x720`,
        
        // Frame 2: Overlay preto (0% a 30% opacidade) a partir do frame 60
        `drawbox=x=0:y=0:w=iw:h=ih:color=black@0.3:t=fill:enable='between(n,${frame2Start},${totalFrames})'`,
        
        // Tag "EXCLUSIVE" - aparece no frame 60
      ...
      ].filter(Boolean)) // Remove filtros vazios
      .size('1280x720')
      .output(videoPath)
      .on('end', () => resolve(`/public/${id}.mp4`))
      .on('error', reject)
...
export async function createAdvancedStoryVideo(
  ...
  // Divide o texto em 3 partes aproximadamente iguais
